...
This script processes and ranks job evaluations with an ASCII gradient display
using Rich. It selects the proper model based on provider mode and supports
three recipe modes:
  • scoring   – processes only new job listings.
  • rescoring – re-evaluates previously scored jobs.
  • all       – concurrently runs both scoring and rescoring.
"""
...
from .graphics import render_evaluation
from .logger import logger, set_verbose

console: Console = Console()
retry_map: Dict[Any, float] = {}

# Use a semaphore for concurrency control (1 job at a time by default).
CONCURRENCY = 1
...
async def evaluate_and_update_job(
    ...
    """
    Evaluate a single job under concurrency control, update its scores and record
    the scoring_model used, and return the updated job.
    """
...
async def process_and_rank_jobs(conf: Dict[str, Any], verbose: bool, threads: int, rescore: bool) -> List[Dict[str, Any]]:
    """
    Fetch job IDs (using scoring or rescoring query), then process each job under controlled concurrency.
    Returns a list of updated job records.
    """
    ...
    try:
        ...
        while True:
            if recipe == "scoring":
                ...
                # Run scoring and rescoring in parallel
