...
class LinkedInScraper:
    ...
    async def fetch_applicants_count(self, context_obj: Any) -> Optional[int]:
        ...
        try:
            ...
            for pattern in patterns:
                if (match := re.search(pattern, full_page_text, re.IGNORECASE)):
                    return int(match.group(1))
        except Exception:
            pass
    ...
    async def extract_job_infos(self, page: Any) -> List[Dict[str, Any]]:
        ...
        for card in job_cards:
            ...
            for sel in company_selectors:
                if (elem := await card.query_selector(sel)) and (company := (await elem.text_content()).strip()):
                    break
            loc = ""
            for sel in location_selectors:
                if (elem := await card.query_selector(sel)) and (loc := (await elem.text_content()).strip()):
                    break
            descr = ""
            for sel in snippet_selectors:
                if (elem := await card.query_selector(sel)) and (descr := (await elem.text_content()).strip()):
                    break
    ...
    async def extract_job_id(self, card: Any) -> Optional[str]:
        for attr in self.job_id_attrs:
            if (val := await card.get_attribute(attr)):
                return val
        return None
    ...
    async def get_job_details(self, page: Any, job_id: str, conn: Any = None, **kwargs) -> Optional[Dict[str, Any]]:
        """
        Retrieves detailed job information. The `conn` parameter is optional;
        if provided, DB updates (such as marking a job as closed) are performed.
        Extra keyword arguments (like search_card_info) are accepted but ignored.
        """
        ...
        if feedback_elem:
            ...
            if closed_app_text in feedback_text:
                ...
                if conn:
                    ...
                    conn.commit()
                return None  # Do not process further, but update DB if possible
    ...
    def parse_posting_date(self, posted_time: Optional[str]) -> str:
        ...
        try:
            ...
            if "day" in pt:
                return (datetime.now() - timedelta(days=int(re.search(r"(\d+)", pt).group()))).strftime("%Y-%m-%d")
            if "week" in pt:
                return (datetime.now() - timedelta(weeks=int(re.search(r"(\d+)", pt).group()))).strftime("%Y-%m-%d")
        except Exception as e:
    ...
    if debug_job_url:
        ...
        async with semaphore:
            ...
            try:
                ...
                # In debug single-link mode, DB update isn’t necessary.
        ...
        for query in search_queries:
            ...
            for info in job_infos:
                ...
                if job_exists(conn, job_url):
                    ...
                    continue
                # Pass the DB connection so that any DB updates in get_job_details are performed.
...
async def fetch_job_detail_task(scraper: LinkedInScraper, info: Dict[str, Any], conn: Any, parent_context: Any, blacklist: Dict[str, set], sem: asyncio.Semaphore) -> Optional[Dict[str, Any]]:
    async with sem:
        ...
        try:
            job_id = info["job_id"]
            # Pass the connection along with extra keyword arguments.
