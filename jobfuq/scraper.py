...
async def run_scrape(config: Dict[str, Any], browser: Any, search_queries: List[Dict[str, Any]], manual_login: bool,
                     endless: bool = False, args: Optional[argparse.Namespace] = None, playwright=None) -> None:
    """
...
async def get_jobcards(config: Dict[str, Any], browser: Any, search_queries: List[Dict[str, Any]],
                       manual_login: bool, args: Optional[argparse.Namespace], playwright=None) -> Any:
...
async def fetch_job_detail_task(scraper: LinkedInScraper, info: Dict[str, Any], conn: Any,
                                parent_context: Any, blacklist: Dict[str, set],
...
async def main_scraper(args: argparse.Namespace) -> None:
    ...
    squeries = config.get("search_queries", [
    ...
    async with async_playwright() as p:
        ...
        if "scrap" in recipe:
            tasks.append(
                asyncio.create_task(
                    run_scrape(config, browser, squeries, args.manual_login, endless=args.endless, args=args, playwright=p)
                )
            )
