WITH RankedJobs AS (SELECT jl.id,
                           jl.company,
                           jl.title,
                           jl.description,
            ...
            AND NOT EXISTS (SELECT 1
                            ...
                            WHERE bl.type = 'blacklist'
                              ...
                              AND NOT EXISTS (SELECT 1
                                              FROM blacklist wl
                                              WHERE wl.type = 'whitelist'
                                                AND LOWER(jl.title) LIKE '%' || LOWER(wl.value) || '%'))) AS total_jobs
FROM RankedJobs
