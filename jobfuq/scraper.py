...
async def evaluate_job(ai_model, job):
...
async def main_scraper(args):
    ...
    async with async_playwright() as p:
        ...
        recipe = args.recipe.split(',') if args.recipe else ['scrape', 'process']
        tasks = []
        if 'scrape' in recipe:
            tasks.append(
                asyncio.create_task(
                    run_scrape(config, browser, squeries, args.manual_login, endless=args.endless, args=args, playwright=p, status=status)
                )
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--recipe', type=str, default='scrape', help="Run mode: 'scrape', 'process', or 'scrape,process'")
