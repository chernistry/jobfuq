...
class OpenRouterModel:
    """
    A model that uses OpenRouter API for chat completions.

    It dynamically updates its rate limits by querying the API and enforces a sliding-window rate limiter.
    """
    ...
    async def _update_rate_limit(self) -> None:
        ...
        This function makes a GET request to the OpenRouter auth endpoint to fetch rate limit info.
        """
        try:
    ...
    async def _rate_limit(self) -> None:
        ...
        Updates the rate limit and sleeps if the number of requests in the current window exceeds the limit.
        """
